...
#define Y 5	//二维数组的y值
	...
	void button(int x, int y, int w, int h, const char* text)//绘制一个按钮
	{
		setbkmode(0);
		setfillcolor(BROWN);
	...
	void success()
	{
		...
		cleardevice();//清屏
		button(50, 50, 550, 50, "成功,点击进入下一关");//调用按钮函数
		...
		while (1) {
			...
			switch (m.message) {
			case WM_LBUTTONDOWN:
				if (m.x >= 50 && m.x <= 600 && m.y >= 50 && m.y <= 100)//鼠标点击到此位置
				{
					LEVEL_2::level2(); //进入第二关
				}
			}
		}
	}
	...
	void fail()
	{
		...
		BeginBatchDraw();//开启双缓冲绘图
		cleardevice();//清屏
		button(50, 50, 550, 50, "失败，点击返回主页");//调用按钮函数
		EndBatchDraw();//结束双缓冲绘图
		ExMessage m;//获取鼠标信息
		while (1) {
			m = getmessage(EX_MOUSE);
			switch (m.message) {
			case WM_LBUTTONDOWN:
				if (m.x >= 50 && m.x <= 50 + 550 && m.y >= 50 && m.y <= 50 + 50)//鼠标点击到此位置
				{
					print_Homepage();//返回主页
				}
			}
	...
	void initMap() {//遍历给每一个数组坐标利用随机数赋值
		for (int i = 0; i < Y; ++i) {
			for (int j = 0; j < X; ++j) {
				map[i][j] = rand() % 4;
			}
	...
	int movedelete() {//消除与下落
		u1 = 1;
		timer0 = 0;
		timer1 = timer2 = timer3 = timer4 = timer5 = 1000000;
		/*初始化boss、方框、精灵*/
		loadimage(&_0, "q1.jpg", 1000, 700);//背景
		loadimage(&_1, "q2.png", 500, 400);//加载消除框
		loadimage(&_2, "q3.png", 500, 400);//加载boss白底
		loadimage(&_3, "q4.png", 500, 400);//加载boss黑底
